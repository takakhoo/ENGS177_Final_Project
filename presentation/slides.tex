...
\author{Dario Blanco Morales \and Even Hogberget \and \\ Kyle D. Ledda-Lewaren \and Taka Khoo}
\institute{ENGS 177 --- Decision Making Under Uncertainty \\ Thayer School of Engineering, Dartmouth College}
\date{May 2026}
...
\medskip
\textbf{Question.} Can a decision-theoretic regime overlay --- a POMDP
solved end-to-end on macroeconomic observables --- beat a static benchmark
...
Fully-observable MDP & Requires knowing the regime --- but it's \emph{latent}. \\
\textcolor{cb}{\textbf{POMDP}} & \textcolor{cb}{Hidden state, Bayesian belief, QMDP for tractable control.} \\
\bottomrule
\end{tabular}

\medskip
\textbf{The course material this exercises:} Lec.~2 Bayes filter; Lec.~3
Markov chain; Lec.~5--7 MDPs (Bellman, VI, PI); Lec.~7 structured policies.
\end{frame}
...
\begin{frame}{HMM regime timeline}
\centering
\includegraphics[height=0.75\textheight,keepaspectratio]{../figures/multistate_regime_timeline.pdf}\\[1pt]
\scriptsize \emph{Worst-mean-SPY regime in red. NBER bars in gray. The 2-state model recovers 2008, 2015--16, 2020--23 stress.}
\end{frame}
...
%-------- Slide 8: Backtest headline --------
\begin{frame}{Backtest 2003--2026, headline}
\centering
\small
...
risk-tolerance, not regime detection, drives the result. Myopic
12-month trend follower wins on risk-adjusted terms.
\end{frame}

%-------- Slide 9: Gamma sweep --------
\begin{frame}{Sensitivity: QMDP unlocks at $\gamma\ge 8$}
\centering
\includegraphics[width=0.78\linewidth]{../figures/gamma_sensitivity_sharpe.pdf}\\[1pt]
\small \emph{QMDP Sharpe crosses static-60/40 at $\gamma\approx 8$; saturates near 0.89 by $\gamma=15$.}

\medskip
\textbf{However:} $\pi^\ast(\text{bull})=\pi^\ast(\text{bear})$ at \emph{every} $\gamma$ we tested. The gains are risk-tolerance shifts, not regime tilts.
\end{frame}
