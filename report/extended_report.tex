...
\textbf{Update step:} ``A high card just came out---more likely Deck A''
\[
...
\subsection{Why our QMDP collapsed at $\gamma{=}2$: ``a gambler with little risk aversion bets the same way regardless of suspicion''}

The $Q^\ast$ table above (with regime-differentiated actions) appears only at $\gamma \geq 8$ in our calibrated MDP. At the headline $\gamma = 2$, every column of $Q^\ast$ has ``100\% stocks'' as the maximum. So no matter what the belief, $\bel^\top Q^\ast(\cdot, 100/0)$ is the highest. QMDP always picks 100\% stocks.

\textbf{Analogy:} a gambler with low risk aversion has so much faith in expected value over downside that even when 93\% suspicious it's the ``bad deck,'' the long-run discounted continuation value of the high-EV action still wins. Only a sufficiently risk-averse gambler ($\gamma \geq 8$) lets the suspicion shift the policy.

\subsection{Why richer observations fix it: ``more thermometers, better diagnosis''}

Two thermometers (VIX, term spread) measure the same broad axis: how spooked is the equity market. They both rise during stress; they don't add independent information about the kind of stress.

\textbf{Adding NFCI and STLFSI4} is like giving the doctor a blood test and an X-ray. NFCI integrates equity, bond, money-market, and shadow-banking signals. STLFSI4 aggregates 18 financial-stress indicators. They move \emph{differently from} VIX in subtle ways (e.g., during the 2022 inflation shock, VIX was moderate but NFCI/STLFSI spiked sharply). With these channels, the HMM can identify a richer bear regime---one where the conditional bond return is meaningfully higher than the conditional stock return, sharp enough that even a low-$\gamma$ MDP shifts toward bonds.

Our cohort study (\Cref{sec:feature-cohort}) confirms this: with VIX+spread only, the regime label is non-actionable. Add NFCI+STLFSI4, and the same $\gamma{=}2$ MDP produces a fully regime-differentiated policy (100/0 in bull, 0/100 in bear).

\subsection{Why walk-forward refit fixes it: ``re-reading the casino's recent hands''}

A fixed HMM is like a player who learned the casino's house edges twenty years ago and never updated. The casino has changed since then---new dealers, new decks, different switching probabilities. Strategies based on stale parameters mis-time their bets.

\textbf{Walk-forward expanding-window refit} re-estimates the HMM every year on all data observed up to that point. The HMM tracks slow drift in the regime moments (e.g., post-2008 the bear-state mean return became less negative as central banks intervened earlier). With proper refit, QMDP's Sharpe lifts from 0.81 to 1.08 in our backtest---a 33\% improvement just from re-reading the recent hands.

\subsection{Why Faber wins: ``surf the wave instead of predicting which wave is coming''}

\textbf{Time-series momentum / Faber 10-month moving-average:} hold an asset if its price is above its 10-month average, else move to cash. No regime model, no Bayesian filter, no MDP. Just: ``is the trend my friend right now?''

Why it works: trend-following exploits \emph{persistence in returns} (momentum), which is a stylised fact across centuries (Hurst--Ooi--Pedersen 2017). It is the empirical sweet spot of robustness vs simplicity. Our 12-strategy horse race confirms: Faber dominates Sharpe 1.68 vs static 0.81 vs QMDP 0.73. The simpler model wins.

\textbf{The lesson:} regime-aware allocation is theoretically appealing but empirically fragile. Even with all our diagnostic unlocks (richer observations, walk-forward refit, higher $\gamma$), our best QMDP variant achieves Sharpe 1.08---competitive with trend-following but not dominant.

\subsection{The honest verdict}

We do not claim that POMDP asset allocation beats simple trend-following. We claim:
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt]
  \item The POMDP framework is the right \emph{minimum} tool for our decision problem (Powell's taxonomy positioning).
  \item Our headline negative finding is a diagnostic of methodology, not fundamentals: it is reversed in isolation by richer observations, walk-forward refit, or higher CRRA.
  \item The HMM signal itself is informative (HMM-MV and BL+HMM both beat QMDP); QMDP's projection at low $\gamma$ destroys it.
  \item Trend-following is the bar that any decision-theoretic regime overlay must clear. We did not clear it.
\end{enumerate}
