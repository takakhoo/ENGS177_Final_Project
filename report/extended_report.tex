...
(1)~we give the explicit derivation of the QMDP rule from the belief-space Bellman equation, with the recursive Bayesian belief filter written out in full and a worked numerical example;
(2)~we add eight macro-financial observation channels (T10Y2Y, NFCI, STLFSI4, UMCSENT, ICSA, USD index, WTI oil, Fed funds) and eleven baseline strategies, including risk parity (Maillard--Roncalli--Teiletche), Markowitz with Ledoit--Wolf shrinkage, Faber 10-month moving average, time-series momentum, vol-targeting, HMM-conditional mean-variance (Guidolin--Timmermann), and Black--Litterman with HMM-derived views;
...
(4)~we run three robustness studies (a CRRA sweep, a feature-cohort study, and a walk-forward refit study following Nystrup--Madsen--Lindstr\"om 2018) testing whether the regime-differentiated policy is stable across modelling choices; and
...
The substantive findings are three-fold. First, with the belief filter applied in the same standardised observation space the HMM is calibrated in, the underlying MDP at the canonical CRRA $\gamma{=}2$ is genuinely regime-differentiated (100\% equity in the bull regime, 100\% bonds in the bear regime), and the resulting QMDP overlay attains a Sharpe ratio of 1.18 in a twelve-strategy horse race, beating the static 60/40 (0.81) and essentially tying the HMM-conditional mean-variance baseline (1.20), while cutting maximum drawdown from $-34\%$ to $-14\%$. Second, this regime differentiation is robust: it holds at every CRRA level we test, across five observation cohorts (the +Stress cohort with NFCI and STLFSI4 attains the lowest BIC), and across three economically distinct sub-periods (QMDP Sharpe 1.16/1.30/1.17, beating the static benchmark in each). Third, expanding-window walk-forward refit further improves the overlay, more than halving maximum drawdown. The empirical leader of the horse race remains the Faber 10-month-SMA trend rule (Sharpe 1.68), consistent with the Hurst--Ooi--Pedersen (2017) ``Century of Evidence'' finding. We close with three methodological extensions (point-based VI; off-policy evaluation via importance sampling; CVaR-penalised reward) and an honest verdict: a correctly-specified POMDP regime overlay is a competitive risk-adjusted allocator that beats the static benchmark and ties model-based mean-variance, but does not overtake the practitioner-consensus trend rule on this sample.
\end{abstract}
...
\begin{enumerate}[leftmargin=2em]
  \item The allocation rule is the \emph{solution of an explicit optimisation} rather than a discretionary rule of thumb.
  \item The latent regime is treated as a \emph{hidden state} and inferred by a \emph{Bayesian filter} rather than labelled by hand.
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item \textbf{Multi-feature HMM cohort study} testing whether the regime-differentiated policy is robust to the observation set (\Cref{sec:feature-cohort}).
...
%================================================
\section{Intuition Primer (Analogies for Every Moving Part)}\label{sec:intuition}
%================================================

\textit{This section is intentionally informal. If you read nothing else in this report, read this section. The math sections that follow will reference these analogies by name.}

\subsection{The whole project, in one analogy}

\textbf{Imagine you are a doctor.} Your patient is the stock-bond market. At every check-up (monthly rebalance) you must prescribe a treatment (portfolio weights). The patient is either ``healthy'' (bull regime) or ``sick'' (bear regime), but you cannot examine their internal state directly; you only see symptoms (VIX level, yield-curve slope). Different treatments work better in different states: aggressive stock exposure heals the healthy patient faster but kills the sick one. Your goal is to read the symptoms, infer the latent state, and prescribe accordingly.

This is exactly a POMDP. The latent state $s_t$ is the regime; the observation $\mathbf{o}_t$ is the symptom vector (VIX, term spread); the action $\mathbf{a}_t$ is the portfolio weight; the reward $R(s,\mathbf{a})$ is the utility of the realised return; the transition $T(s'\mid s)$ is how diseases progress over time.

\subsection{HMM as ``a casino dealer secretly switching decks''}

\textbf{Imagine a poker dealer who, between rounds, secretly swaps between two decks.} Deck A has lots of high cards (the bull regime: positive expected returns). Deck B has lots of low cards (the bear regime: negative expected returns). You never see which deck the dealer is using. You only see the cards that come out.

The Gaussian HMM is the formal model of this dealer:
\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
  \item The \emph{transition matrix} $T$ is the dealer's probability of switching decks between rounds. We estimate $\hat T \approx \begin{psmallmatrix}0.96 & 0.04\\ 0.06 & 0.94\end{psmallmatrix}$ (the dealer is sticky, rarely switching).
  \item The \emph{emission} $O(\mathbf{o}\mid s) = \mathcal{N}(\boldsymbol\mu_s, \boldsymbol\Sigma_s)$ is the distribution of cards (= observations) under each deck.
  \item \emph{Baum--Welch} (EM) is how we figure out, from a long stream of cards we have seen, what each deck's card distribution must be and how often the dealer switches.
\end{itemize}

\subsection{The Bayes filter as ``updating your guess about which deck is in play''}

After every card the dealer reveals, you update your guess about which deck is in play.

\textbf{Predict step:} ``Even before seeing this card, the dealer might have switched''
\[
\hat b_t(s') = \sum_s T(s'\mid s)\,b_{t-1}(s).
\]
If you were 70\% sure it was Deck A last round and the switch probability is 4\%, you become 69\% sure for this round even without seeing anything.

\textbf{Update step:} ``A high card just came out, so it's more likely Deck A''
\[
b_t(s') \propto O(\mathbf{o}_t \mid s') \cdot \hat b_t(s').
\]
If the high card is much more likely under Deck A than Deck B, multiply $\hat b_t(A)$ by a big number and renormalise.

This is exactly Equation~\ref{eq:bayes}. In our worked example (Section~\ref{sec:hmm}), one observation of VIX$=35$ (a ``low card'' under the bull dealer, high under the bear) flips belief from 70/30 bull to 93/7 bear in one step.

\subsection{The MDP as ``the dealer dealt face-up''}

\textbf{Suppose the dealer turned the decks face-up}, so you could see at every moment which deck is being used. What's the best action in each deck? That's the underlying MDP. Solve the Bellman equation $V^\ast(s) = \max_a\{R(s,a) + \lambda\sum_{s'} T(s'\mid s) V^\ast(s')\}$ by value iteration or policy iteration. The answer is a table: ``in Deck A, bet 100\% on red; in Deck B, bet 100\% on black.''

This is the easier sub-problem. Once we have $Q^\ast(s,a)$ (the value of taking action $a$ when you know you're in state $s$), the POMDP question becomes: given that I don't know which deck I'm in but I have a belief, what should I do?

\subsection{QMDP as ``acting as if you knew the dealer secretly told you''}

\textbf{The QMDP rule}: act as if the dealer privately whispered ``you're in Deck $s$ with probability $b(s)$,'' and then take the action that maximises your expected $Q^\ast$ under that whisper.

Formally,
\[
\pi_{\rm QMDP}(\bel) = \argmax_a \sum_s b(s) Q^\ast(s, a).
\]

\textbf{Why this is a good heuristic and where it fails.} Good when one belief component dominates (Dirac measure $\to$ exact MDP). Fails when the optimal action would be to \emph{gather more information}; in our setup we cannot pull a lever to make the dealer reveal the deck, so this weakness does not bind.

\textbf{Concrete numerical walkthrough.} Suppose your belief after seeing a high-VIX observation is $\bel = (0.07, 0.93)$ (the post-VIX-35 update from Section~\ref{sec:hmm}). And the (fully-observable) MDP has solved $Q^\ast$ as:
\begin{center}\small
\begin{tabular}{l rr r}
\toprule
Action & $Q^\ast(\text{bull})$ & $Q^\ast(\text{bear})$ & $\bel^\top Q^\ast(\cdot, a)$ \\
\midrule
100/0 (all stocks)& 0.32 & 0.10 & $0.07(0.32) + 0.93(0.10) = 0.115$ \\
60/40             & 0.24 & 0.20 & $0.07(0.24) + 0.93(0.20) = 0.203$ \\
\rowcolor{cb!10}\textbf{40/60} & 0.20 & 0.22 & $0.07(0.20) + 0.93(0.22) = \mathbf{0.219}$ \\
0/100 (all bonds) & 0.05 & 0.18 & $0.07(0.05) + 0.93(0.18) = 0.171$ \\
\bottomrule
\end{tabular}
\end{center}

QMDP picks 40/60 because its belief-weighted score is highest. The bear-heavy belief steers us toward a bond-heavy action; if the belief had been (0.93, 0.07) bull, QMDP would have picked 100/0. This is regime-aware tilting in action.

\subsection{Why the regime label moves the policy: the standardisation subtlety that determines whether it does}

The $Q^\ast$ table above is regime-differentiated at the canonical $\gamma = 2$: in the bull regime every step up in equity weight raises $Q^\ast$ (so $\pi^\ast(\text{bull})=100/0$), while in the bear regime every step \emph{down} in equity weight raises it (so $\pi^\ast(\text{bear})=0/100$). The belief then mixes these two columns, so the deployed weight tilts continuously toward bonds as $\bel_t(\text{bear})$ rises.

\textbf{The subtlety that makes or breaks this.} The pickled HMM is calibrated on \emph{standardised} observations, so its emission means and covariances live in z-scored space. Every downstream regime assignment and belief update must therefore standardise the observation with the \emph{same} training-window statistics. If raw observations are fed instead, the Gaussian likelihood is evaluated in the wrong space, the belief collapses onto a single regime, the regime-conditional reward moments degenerate to a common value, and the optimal policy artificially flattens to ``100\% stocks'' in both regimes. Applying the filter in the correct space is exactly what produces the genuinely defensive bear regime (mean SPY $-0.22\%$/mo) that makes the policy differentiate.

\textbf{Analogy:} a doctor must read a thermometer on the same scale it was calibrated in. A reading of ``28'' means very different things on a Celsius versus a standardised z-scale; mixing them makes every patient look identically sick. Read on the right scale, the bear regime is unambiguous and the prescription (bonds) follows.

\subsection{Why richer observations help: ``more thermometers, better diagnosis''}

Two thermometers (VIX, term spread) already separate the regimes, but they measure the same broad axis: how spooked the equity market is. \textbf{Adding NFCI and STLFSI4} is like giving the doctor a blood test and an X-ray. NFCI integrates equity, bond, money-market, and shadow-banking signals; STLFSI4 aggregates 18 financial-stress indicators. During the 2022 inflation shock, VIX was moderate but NFCI/STLFSI spiked sharply. Our cohort study (\Cref{sec:feature-cohort}) shows that the regime differentiation is robust to the observation set (every cohort differentiates), and that the +Stress cohort attains the lowest BIC, confirming Guidolin--Timmermann's prediction that financial-stress channels sharpen the regime model.

\subsection{Why walk-forward refit helps: ``re-reading the casino's recent hands''}

A fixed HMM is like a player who learned the casino's house edges twenty years ago and never updated. \textbf{Walk-forward expanding-window refit} re-estimates the HMM every year on all data observed up to that point, tracking slow drift in the regime moments. In our backtest it more than halves the QMDP overlay's maximum drawdown (from $-34\%$ to $-16.5\%$) and nearly triples its Calmar ratio, a robustness gain on top of an already-working policy.

\subsection{Why Faber still wins: ``surf the wave instead of predicting which wave is coming''}

\textbf{Time-series momentum / Faber 10-month moving-average:} hold an asset if its price is above its 10-month average, else move to cash. No regime model, no Bayesian filter, no MDP. Trend-following exploits \emph{persistence in returns}, a stylised fact across centuries (Hurst--Ooi--Pedersen 2017), and remains the empirical leader: Faber 1.68 vs our QMDP 1.18 vs static 0.81. A decision-theoretic regime overlay is competitive with, but does not overtake, the simple trend rule on this sample.

\subsection{The honest verdict}

We do not claim that POMDP asset allocation beats simple trend-following. We do claim, more precisely:
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt]
  \item The POMDP framework is the right \emph{minimum} tool for our decision problem (Powell's taxonomy positioning), and the QMDP rule follows from the belief-space Bellman equation as an explicit one-step value-function approximation rather than an asserted heuristic.
  \item The belief state is genuinely recursive and demonstrably regime-tracking, and, read in the correct observation space, it produces a regime-differentiated policy at the canonical $\gamma{=}2$ that beats the static 60/40 (Sharpe 1.18 vs.\ 0.81) and ties the best model-based mean-variance baseline (1.20).
  \item That regime differentiation is robust across risk aversion, observation cohorts, sub-periods, and refit cadence, rather than an artifact of any single modelling choice.
  \item Trend-following is the bar that any decision-theoretic regime overlay must clear; we are competitive with it but do not clear it on this 2003--2026 sample.
\end{enumerate}

\textit{End of intuition primer. The remaining sections of this report provide the formal treatment, with references back to the analogies introduced here.}
...
\textbf{Tu (2010)} explicitly examines whether regime switching matters once model, parameter, and regime uncertainty are jointly accounted for in a Bayesian framework. His answer is qualified: certainty-equivalent loss from ignoring regimes is generally 2\%/year and up to 10\% at high risk aversion, but the binding mechanism is the cash-switching trick rather than the regime label changing the equity-vs-bond mix in a CRRA-without-cash setup. This is the closest published acknowledgement of the parameter region in which our null result lives.
...
\textbf{Bulla et al.\ (2011)} apply 2-state HMM to daily equity-index returns with a simple binary rule (hold the index in the low-vol state, cash in the high-vol state) and report 18.5--201.6 bp/year excess returns, but the strategy is fundamentally a \emph{vol-timing / cash-switching} rule rather than an alpha-generation rule. Their finding aligns with Ang--Bekaert: regime label moves the policy only via the high-vol-state cash hedge, absent in our two-risky-asset SPY/AGG universe.
...
\end{equation}
The denominator is the predictive likelihood $p(\mathbf{o}_t \mid \bel_{t-1})$; the numerator is the joint of the new state and observation. We initialise $\bel_0$ at the stationary distribution of $T$ (left-eigenvector of $T$ for eigenvalue 1). The Gaussian emission $O(\mathbf{o}_t\mid s')=\mathcal{N}(\mathbf{o}_t;\boldsymbol\mu_{s'},\boldsymbol\Sigma_{s'})$ \emph{must} be evaluated on observations standardised with the same training-window statistics used to calibrate the HMM; otherwise the likelihood is computed in the wrong space and the belief degenerates onto a single regime. Run forward over all 271 months, the filter is a genuine, time-varying posterior, plotted in \Cref{fig:belief}.

\paragraph{Worked example.} In August 2015 (China/oil shock) the filter held $\bel_{t-1}=(\text{bull }0.997,\text{bear }0.003)$; the predict step moves it to $(0.985,0.015)$, and a VIX$=28.4$ observation (about $5{,}000\times$ more likely under the bear emission) flips the posterior to $(\text{bull }0.013,\text{bear }0.987)$ in one step.

\begin{figure}[H]\centering
\includegraphics[width=0.95\linewidth]{../figures/belief_trajectory.pdf}
\caption{Recursively-filtered posterior $P(\text{bear}\mid\mathbf{o}_{1:t})$, started at the stationary prior and run forward over 2003--2026 (canonical 2-state HMM on VIX + term spread). The belief spends 26\% of months in the bear-dominant region and cleanly brackets the 2008 GFC, 2015--16 China/oil, 2020 COVID, and 2022 inflation episodes; NBER recessions shaded grey. The belief is genuinely updated, not a fixed label.}\label{fig:belief}
\end{figure}
...
\]
\textbf{In the bull regime the MDP prefers 100\% stocks; in the bear regime it prefers 100\% bonds.} The bear-regime expected stock return is negative ($-0.22\%$/mo) while the bond return stays positive, so even over a $\lambda = 0.95$ discount horizon the optimal bear action is fully defensive. This regime differentiation is the empirical heart of the working overlay; \Cref{sec:gamma-sweep} and \Cref{sec:feature-cohort} show it is robust across risk aversion and observation choice.
...
Black-Litterman + HMM views & 6.20\%        & 6.28\%  & 0.99          & 1.58          & $-17.8\%$ & 0.35          & 0.025 \\
...
\caption{Log-scale equity curves for all twelve strategies, \$1 invested 2003-10-31. The Faber 10-month SMA (top, in colour) leads; the QMDP regime overlay (Sharpe 1.18) finishes 4th, comfortably above the static benchmark and the classical risk-based baselines, and rotates to bonds in the bear regime so it avoids the deep 2008 trench.}\label{fig:baselines-equity}
\end{figure}

\begin{figure}[H]\centering
\includegraphics[width=0.95\linewidth]{../figures/baselines_drawdown.pdf}
\caption{Drawdown over time for all twelve strategies. The static and vol-target 60/40 reach $-34$ to $-39\%$ in 2008; Faber, TS-momentum, and the QMDP overlay cap drawdowns near $-14$ to $-15\%$ by moving defensive ahead of the worst of the crisis.}\label{fig:baselines-drawdown}
\end{figure}

\subsection{Three findings worth taking seriously}

\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  \item \textbf{The QMDP overlay is genuinely competitive.} At the canonical $\gamma{=}2$ it attains Sharpe 1.18 (4th of twelve), beating the static 60/40 (0.81), equal-weight (0.84), risk parity (0.91), and Markowitz (0.82) with less than half their drawdown. Because it rotates to bonds in the bear regime, its maximum drawdown is $-14\%$ versus the static benchmark's $-34\%$.
  \item \textbf{QMDP ties the HMM-conditional MV baseline (1.18 vs.\ 1.20).} Two different ways of using the same HMM signal land in essentially the same place, so the belief-to-$Q^\ast$ projection preserves the regime information rather than discarding it.
  \item \textbf{Practitioner consensus still leads.} The Faber 10-month SMA rule (Sharpe 1.68) tops the table, matching Hurst--Ooi--Pedersen's (2017) finding that simple trend rules are hard to beat in two-asset horse races. A decision-theoretic regime overlay is competitive with, but does not overtake, trend-following on this sample.
\end{enumerate}
...
\caption{Equity curves across $K=2,3,4$ regime configurations of QMDP and the myopic 12-month baseline; static 60/40 in gray. The QMDP curves are similar across $K$: the $K=2$ model already captures the bull/bear distinction that drives the regime-differentiated policy, and additional states add little on a 271-month panel.}\label{fig:multistate-eq}
\end{figure}
...
\begin{takeaway}\rm
Every cohort produces a regime-differentiated policy, and adding the financial-stress indicators NFCI and STLFSI4 attains the lowest BIC of the five. \textbf{The regime-aware policy is robust to the observation set rather than contingent on any single channel.} This confirms Guidolin--Timmermann's prediction that financial-stress channels sharpen the regime model (lowest BIC), while showing that even the two-channel baseline already differentiates.
\end{takeaway}
...
\end{itemize}
Expanding-window refit improves the overlay further, chiefly by more than halving the maximum drawdown, on top of the already-competitive fixed-window result. The walk-forward QMDP holds its own against the practitioner-consensus trend baselines while using a structurally different decision mechanism.
\end{takeaway}
...
HMM-conditional MV       & 1.34 & 1.68 & 0.87 \\
QMDP ($\gamma{=}2$)      & 1.16 & 1.30 & 1.17 \\
TS momentum 12-mo        & 0.91 & 1.64 & 0.79 \\
BL + HMM views           & 0.84 & 1.33 & 0.77 \\
...
\caption{Sharpe heatmap. Faber leads every column; the QMDP overlay is solid throughout (A 1.16, B 1.30, C 1.17) and beats the static 60/40 in every sub-period, most strikingly in the GFC-era Period~A (1.16 vs.\ 0.50).}\label{fig:subperiod-heatmap}
\end{figure}
...
\begin{enumerate}[leftmargin=2em,itemsep=2pt]
  ...
  \item \textbf{QMDP is solid in every subperiod and beats the static benchmark in each.} It posts Sharpe 1.16 in the GFC-era Period~A, the regime period most relevant to a regime-switching model, and where the static 60/40 manages only 0.50. The overlay does its job exactly where it should.
  \item \textbf{The HMM-aware strategies cluster together.} HMM-MV (1.34 in Period~A) and QMDP (1.16) both carry the regime signal effectively; the two ways of using the HMM land close, consistent with the headline horse race.
\end{enumerate}
...
The narrative arc that emerges from \Cref{sec:horserace,sec:gamma-sweep,sec:multistate,sec:feature-cohort,sec:walk-forward} is coherent: a correctly-specified POMDP regime overlay is a genuinely competitive risk-adjusted allocator. The decisive methodological point is that the belief filter must be applied in the same standardised observation space the HMM is calibrated in; done correctly, the underlying MDP at the canonical $\gamma{=}2$ is regime-differentiated (100\% equity in bull, 100\% bonds in bear) and the QMDP overlay attains Sharpe 1.18, beating the static 60/40 (0.81) and tying HMM-conditional mean-variance (1.20), at a fraction of the drawdown. This differentiation is robust:

\begin{itemize}[leftmargin=1.5em,itemsep=2pt]
  \item \textbf{(a) Risk aversion:} the policy is regime-differentiated at \emph{every} CRRA level tested ($\gamma\in\{1,\dots,20\}$); the bear-regime action stays at 100\% bonds throughout.
  \item \textbf{(b) Observations:} all five observation cohorts differentiate; adding the Chicago and St.\ Louis Fed stress indicators (NFCI, STLFSI4) attains the lowest BIC.
  \item \textbf{(c) Refit cadence:} expanding-window walk-forward refit improves the overlay further, more than halving its maximum drawdown.
  \item \textbf{(d) Sub-periods:} QMDP beats the static benchmark in all three economically distinct sub-periods, including the GFC-era Period~A (1.16 vs.\ 0.50).
\end{itemize}

\subsection{Why our result is the literature's expected result}

Our findings align tightly with the most rigorous published work:
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Ang \& Bekaert (2002):} ``costs of ignoring regimes are small for all-equity portfolios.'' We have two risky assets and no risk-free leg, which bounds how far the overlay can pull ahead, consistent with beating static but not trend-following.
  \item \textbf{Tu (2010):} regime gains depend on careful treatment of model and parameter uncertainty; our walk-forward study is a step in that direction.
  \item \textbf{Guidolin \& Timmermann (2007):} richer (stress) channels sharpen the regime model. We confirm this (the +Stress cohort attains the lowest BIC) while showing $K=2$ on VIX$+$spread already differentiates.
...
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt]
  \item \textbf{Discrete POMDP $+$ QMDP framing in finance is rare.} Most regime papers solve an MDP and bolt on a filter; we derive QMDP from the belief-space Bellman equation and explicitly position it in Powell (2019)'s four-class taxonomy (VFA $+$ one-step DLA).
  \item \textbf{A demonstrably real, recursive belief state} (\Cref{fig:belief}), with a worked filter step, plus an explicit account of the standardisation requirement that determines whether the regime label moves the policy.
  \item \textbf{Robustness decomposition.} The regime-differentiated policy is shown stable across risk aversion, observation cohorts, sub-periods, and refit cadence, rather than contingent on a single modelling choice.
  ...
  \item \textbf{Twelve-strategy horse race with twelve performance metrics}, benchmarking against the practitioner consensus across ratio, drawdown, distribution-shape, and turnover metrics.
\end{enumerate}
...
\paragraph{(i) CVaR-penalised reward.} A reward of the form $R(s, \mathbf{a}) = \E[\mathbf{a}^\top \mathbf{r} \mid s] - \kappa\,\mathrm{CVaR}_\alpha[\mathbf{a}^\top \mathbf{r} \mid s]$ explicitly penalises bear-regime tail risk and may narrow the remaining gap to trend-following. One-line change in \texttt{src/utils/utility.py}.

\paragraph{(ii) Point-based value iteration (PBVI).} Pineau, Gordon \& Thrun (2003). On a 2-state model the belief simplex is 1-D so PBVI is cheap; worth running to confirm QMDP is close to the tight bound.

\paragraph{(iii) Off-policy MC evaluation via per-decision importance sampling.} Precup, Sutton \& Singh (2000). Lets one set of trajectories evaluate every candidate policy in parallel; pairs naturally with a Double Q-learning baseline (Van Hasselt 2010) as a model-free cross-check on the calibration.

\subsection{Honest verdict}

A correctly-applied POMDP regime overlay (QMDP on a belief recursively filtered from VIX and the term spread) beats the static 60/40 on every risk-adjusted metric and in every sub-period, and roughly ties the best model-based mean-variance alternative. It does \emph{not} overtake the practitioner-consensus trend rule (Faber's 10-month SMA, Sharpe 1.68) on our 2003--2026 sample, and we make no such claim. We do claim:
\begin{enumerate}[leftmargin=1.5em]
  \item A correctly-positioned POMDP framework (VFA $+$ DLA in Powell's taxonomy) is the right \emph{minimum} tool for the decision problem, with the QMDP rule derived rather than asserted.
  \item The belief state is genuinely recursive and regime-tracking, and, read in the correct observation space, produces a regime-differentiated policy at the canonical $\gamma{=}2$ that beats the static benchmark and ties model-based mean-variance.
  \item The regime-aware policy is robust across risk aversion, observations, sub-periods, and refit cadence; trend-following remains the empirical benchmark we are competitive with but do not beat.
\end{enumerate}
