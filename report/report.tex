...
portfolio of two assets --- here a U.S.\ equity index (SPY) and a U.S.\
...
is a Dirac measure, (ii)~it requires only an MDP solver, which is course
material (Lectures 5--7), and (iii)~it scales to the finite-but-large action
grids we use.

\subsection{Relation to the course}

This problem exercises virtually every result from ENGS\,177 simultaneously:
Bayesian belief updates (Lec.~2), the underlying Markov chain over regimes
(Lec.~3), the decision-graph view of one rebalance period (Lec.~4), the
finite- and infinite-horizon MDP machinery (Lec.~5--7), and the structural
results on monotone policies (Lec.~7). It also points forward to multi-armed
bandits (Lec.~8), since a model-free baseline is one of the natural extensions
we discuss in \Cref{sec:conclusions}.
...
observation $\mathbf{o}_t$, the Bayesian update (Lec.~2) is
\begin{align}
\bel_t(s')
...
and is a sensible heuristic when uncertainty is moderate. It under-explores
when the optimal action depends on
\emph{reducing} uncertainty via deliberate information gathering --- not a
concern here, because regimes are exogenous.
...
hold the parameters fixed thereafter --- a known mild source of look-ahead
...
\caption{Filtered probability of each regime, $K\in\{2,3,4\}$. The
worst-mean-SPY-return regime is highlighted in red and is the de-facto
bear state. NBER recession bars (gray) sit cleanly inside the bear bands
for $K=2$. For $K=3$ and $K=4$ the HMM collapses extra states.}
...
\begin{table}[t]
\centering\small
\caption{In-sample regime-conditional moments (2-state HMM).}\label{tab:regime-stats}
\begin{tabular}{lrrrrrr}
\toprule
Regime & Months & Mean SPY (\%/mo) & SD SPY (\%/mo) & Mean AGG (\%/mo) & SD AGG (\%/mo) & Mean VIX \\
\midrule
...
\begin{tabular}{rcccc}
\toprule
$K$ & VI iterations $(\gamma=2)$ & PI iterations & VI iterations $(\gamma=10)$ & PI iterations \\
\midrule
2 & 125 & 2 & 125 & 2 \\
3 & 96  & 2 & 96  & 2 \\
4 & 96  & 2 & 96  & 2 \\
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{../figures/multistate_equity_curves.pdf}
\caption{Cumulative wealth, 2003--2026 (log scale). Equity curves for the
QMDP and myopic policies under $K\in\{2,3,4\}$ regime configurations and
the single Static 60/40 benchmark. QMDP and Static curves are essentially
indistinguishable across $K$ because the underlying MDP collapses to a
100\%-stocks allocation under CRRA $\gamma=2$.}
\label{fig:equity}
\end{figure}
...
risk aversion $\gamma\in\{1,2,3,5,8,10,15,20\}$ and re-solve the MDP for each.
At $\gamma\le 3$ the optimal policy is "100\% stocks" in every regime;
$\gamma=5$ shifts to 80\%/20\%; $\gamma=8$ to 40\%/60\%; $\gamma=15$ to
20\%/80\%; $\gamma=20$ to 20\%/80\%. Crucially, $\pi^\ast(s_{\rm bull})$
equals $\pi^\ast(s_{\rm bear})$ at \emph{every} $\gamma$. That is, the
optimal policy does \emph{not} differ across regimes --- it merely shifts
with the agent's risk-tolerance level. The QMDP Sharpe ratio surpasses the
static-60/40 benchmark Sharpe (0.81) starting at $\gamma=8$ and saturates
near 0.89 at $\gamma\ge15$.
...
\end{subfigure}
\caption{CRRA risk-aversion sweep. (a)~QMDP Sharpe crosses static-60/40
at $\gamma\approx 8$. (b)~At $\gamma=2$ the QMDP policy is all-stocks; by
$\gamma=20$ it is 20/80 in both regimes.}\label{fig:gamma}
\end{figure}

\Cref{tab:gamma-policy} makes this concrete. Within the columns of the table,
the policy is constant across rows --- regime-insensitive --- but it shifts
\emph{column to column} as $\gamma$ rises.

\begin{table}[t]
\centering\small
\caption{Optimal policy $\pi^\ast(s)$ as a function of regime $s$ and CRRA $\gamma$.
The same column is repeated across rows, indicating regime-insensitivity.}
...
End-to-end, this report is also a working example of the course's full
toolkit: a hidden Markov chain (Lec.~3) viewed through the lens of a Bayesian
network (Lec.~2), inserted into a fully-observable Bellman recursion solved
both by value iteration and exact policy iteration (Lec.~7), all wrapped in a
belief-state controller (QMDP). The report's own conclusions then point
forward to model-free reinforcement learning (Lec.~8) as an alternative
worth investigating.
...
Built model of uncertainty (HMM) & 70\% & --- & 20\% & 10\% \\
